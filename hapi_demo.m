...
% vector, so "Column #" is used in the legend. HAPI metadata should
% include an option to add column labels.
server     = url;
dataset    = 'dataset1';
parameters = 'vector';
start      = '1970-01-01';
...
% y-labels tranformmult(:,:,1), transformmulti(:,:,2), and
% transformmulti(:,:,3) and legend labels of "Column #", where # = 1, 2, or 3.
% In this case the data provided intended to provided a time series of 
% rotation matrices with labels (using Javascript array notation)
% [[Txx,Txy,Txz],[Tyx,Tyy,Tyz],[Tzx,Tzy,Tzz]]. HAPI should include the
...
% Here we have a time series of integers that are intended to communicate
% a status represented by a string.  The metadata includes a map from
% an integer to a string, and this map is used to generate y-axis
% labels.
server     = url;
dataset    = 'dataset1';
...
% as vectormulti(:,:,1) and second as vectormulti(:,:,2).
server     = url;
dataset    = 'dataset1';
...
% In this unusual dataset, 3-vector of strings is given. HAPIPLOT creates
% three time series plots, one for each vector component.
server     = url;
dataset    = 'dataset1';
